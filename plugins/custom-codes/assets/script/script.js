...
(function ($) {
	...
	$(document).ready(function () {
		...
		$('form#post').submit(function (e) {
			...
			} else customCodes.unBlockPageChange(); // Allow changing window
		...
		$('#title').on('input', function () {
			...
			if (initialPostTitle == newTitle) customCodes.unBlockPageChange(); // Allow changing window
	...
	$(document).keydown(function (event) {
		...
		if ((event.ctrlKey || event.metaKey) && event.which == 83 && parseInt(customCodesData.commandS)) {
			...
			if (!$('#ajax-saver').length) customCodes.unBlockPageChange(); // Allow changing window
